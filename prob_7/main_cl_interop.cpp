...
#endif
#include <CL/cl_gl.h>
#if cl_khr_gl_sharing == 0
	#error required cl_khr_gl_sharing to compile
#endif
#include <GL/glx.h>
#include <GL/gl.h>
#include <sstream>
...
int main(int argc, char** argv) {
	...
	// confirm GL interop availability for the device
	if (true) {
		size_t ret_size = 0;
		success = clGetDeviceInfo(device()[device_idx], CL_DEVICE_EXTENSIONS, 0, 0, &ret_size);

		if (reportCLError(success)) {
			stream::cerr << "error getting device extensions\n";
			return -1;
		}

		const char sought[] = "cl_khr_gl_sharing";
		const size_t soughtLen = strlen(sought);

		if (ret_size < soughtLen) {
			stream::cerr << "failure confirming device extension cl_khr_gl_sharing\n";
			return -1;
		}

		const scoped_ptr< char, generic_free > buffer(reinterpret_cast< char* >(std::malloc(ret_size)));
		success = clGetDeviceInfo(device()[device_idx], CL_DEVICE_EXTENSIONS, ret_size, buffer(), 0);
		...
		for (i = 0; i < ret_size - soughtLen + 1; ++i) {
			for (j = 0; buffer()[i] == sought[j] && j < soughtLen; ++j, ++i);
	...
	for (size_t i = 0; i < sizeof(tex_name) / sizeof(tex_name[0]); ++i) {
		...
		glBindTexture(GL_TEXTURE_RECTANGLE, tex_name[i]);
		...
		glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_R8UI, image_w, image_h, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, 0);

		glBindTexture(GL_TEXTURE_RECTANGLE, 0);

		if (testbed::util::reportGLError()) {
			stream::cerr << "error at GLtexture setup\n";
			return -1;
		}
	}
	...
	const char source_implicit_mad[] =
		...
		"	uint result = traverse(get_octet(src_a, 0), src_b, src_c, &root_bbox, &ray, &hit);\n"
		"	const uint axis_x = 0x020100;\n"
		"	const uint axis_y = 0x010002;\n"
		"	const uint axis_z = 0x000201;\n"
		"	const int a_mask = -hit.a_mask;\n"
		"	const int b_mask = -hit.b_mask;\n"
		"	const uint axis = (axis_x & a_mask | axis_y & ~a_mask) & b_mask | axis_z & ~b_mask;\n"
#if 0
		"	const int3 axis_sign = (int3)(0x80000000) & hit.min_mask;\n"
		"	const float3 normal = shuffle((float4)(1.f, 0.f, 0.f, 0.f), convert_uint4((uchar4)(axis >> 0, axis >> 8, axis >> 16, axis >> 24))).xyz;\n"
		"	result = -1U != result ? convert_int(max(0.f, dot(as_float3(as_int3(normal) ^ axis_sign), sun)) * 255.f) : 0;\n";
#else
	...
	while (testbed::processEvents(input) && frame != size_t(frames)) {
		...
		success = clEnqueueAcquireGLObjects(queue, 1, &dst_d[frame & 1],
			0, 0, &event_data_set_ready[frame & 1][4]);

		if (reportCLError(success)) {
			stream::cerr << "error enqueuing GLobject acquire for result image\n";
			break;
		}
		...
		if (0 != frame) {
			...
			success = clEnqueueReleaseGLObjects(queue, 1, &dst_d[ready_frame & 1],
				1, &event_kernel_complete[ready_frame & 1], &event_ocl_done_with_frame);

			if (reportCLError(success)) {
				stream::cerr << "error enqueuing GLobject release for result image\n";
				break;
			}

			success = clWaitForEvents(1, &event_ocl_done_with_frame);
			...
			testbed::monv::render(tex_name[ready_frame & 1]);
			testbed::swapBuffers();
