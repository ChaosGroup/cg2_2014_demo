...
int main(int argc, char** argv) {
	...
	const char source_buffer[] =
		...
		"		float nearest_dist = fmin(distance[i], ray_len);\n"
		"		uint voxel_id = -1U;\n"
	...
	const char source_implicit_mad[] =
		...
		"	const uint luma = convert_int(max(0.f, dot(as_float3(as_int3(normal) ^ axis_sign), sun)) * 255.f);\n"
		"\n"
		"	if (-1U != result) {\n"
		"		const float dist = ray.ray.rcpdir.w;\n"
		"		const float3 ray_rcpdir = fmax(fmin(1.0f / sun, MAXFLOAT), -MAXFLOAT);\n"
		...
		"		result = select(0U, luma, -1U == traverse(get_octet(src_a, 0), src_b, src_c, &root_bbox, &occl.ray, &occl.hit));\n"
		"	}\n"
		"	else\n"
		"		result = 0;\n";
#else
