...
#define _PS_CONST_INT(Name, Val) \
	static const int _ps_##Name[4] __attribute__ ((aligned(16))) = { Val, Val, Val, Val }
...
inline v4sf pow_ps(const v4sf x, const v4sf y) {
	const v4sf zero_filter = _mm_cmpeq_ps(x, _mm_setzero_ps());
	return _mm_andnot_ps(zero_filter, exp_ps(_mm_mul_ps(log_ps(x), y)));
}
